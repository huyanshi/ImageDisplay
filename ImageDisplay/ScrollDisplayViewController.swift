...
class ScrollDisplayViewController: UIViewController,UIPageViewControllerDelegate,UIPageViewControllerDataSource {
    ...
    lazy var bannerUrls: [String] = [String]()
    ...
    /// 设置页数的圆点正常颜色默认灰色
    var pageControlColor = UIColor.whiteColor()
    /// 设置页数圆点高亮颜色默认金的......
    var pageControlHightColor = UIColor.blackColor()
    ...
    func configueUI(paths: [String]) {
        let arr = NSMutableArray()
        for var i = 0; i < paths.count; i++ {
            let path = paths[i]
            let btn = UIButton(type: .Custom)
//            btn.sd_setBackgroundImageWithURL(NSURL(string: path), forState: .Normal, placeholderImage: UIImage(named: "profiles_placeholder270x180"))
